...
function performTaxCalculation() {
    ...
    for (let caseIndex = 0; caseIndex < caseCount; caseIndex++) {
        ...
        for (let colType of ["�꾩옱�곹솴", "�꾨��뚮뱷"]) {
            ...
            const elderlyCare = calculateElderlyCare(healthInsurance);
            const pensionBase = Math.min(incomeData.employmentIncome, pensionLimit);
            const nationalPension = calculateNationalPension(result.totalIncome);

            console.log("healthInsurance", healthInsurance);
            console.log("elderlyCare", elderlyCare);
            console.log("nationalPension", nationalPension);
                ...
                if (totalIncomeRow && semiIncreaseRow && semiTaxRow && ratioRow) {
                    ...
                    if (semiIdx !== -1 && semiIncreaseRow.cells[semiIdx]) {
                        semiIncreaseRow.cells[semiIdx].textContent = Number.isFinite(semiDiff)
                            ? Math.round(semiDiff).toLocaleString()
                            : "";
                    }
                    ...
                    if (netIncomeRow) {
                        const netIncomeIdx = findCellIndex(netIncomeRow, rentalIdx);
                        const netIncomeValue = Math.max(incomeRental - incomeCurrent, 0) - semiDiff;

                        if (netIncomeIdx !== -1 && netIncomeRow.cells[netIncomeIdx]) {
                            const finalValue = roundIfDecimal(netIncomeValue);
                            netIncomeRow.cells[netIncomeIdx].textContent = Number.isFinite(finalValue)
...
function calculateElderlyCare(healthInsurance) {
    const CARE_RATE = 0.1295;
    return Math.round(healthInsurance * CARE_RATE);
}
...
//援���곌툑
function calculateNationalPension(income) {
    const PENSION_LIMIT = 74040000;
    const PENSION_RATE = 0.09;
    return Math.round(Math.min(income, PENSION_LIMIT) * PENSION_RATE);
}
...
function calculateFinalTaxWithCredits({
    ...
    const finalTax = Math.round(baseTax * 1.1);
    return finalTax;
}
